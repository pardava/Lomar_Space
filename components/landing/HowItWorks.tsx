        ...
        <div className="mt-16 grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          {steps.map((step, index) => {
            ...
            return (
              <div
                key={step.title}
                className="relative rounded-2xl border border-[#3A2119]/8 bg-[#D2E2EC] p-7"
              >
                <span className="font-[family-name:var(--font-fraunces)] text-sm text-[#F2E6D6]">
                  0{index + 1}
                </span>

                <div className="mt-4 flex h-12 w-12 items-center justify-center rounded-xl bg-[#3A2119] text-white">
                  <Icon size={20} />
                </div>
                ...
                {index < steps.length - 1 && (
                  <div className="absolute right-0 top-1/2 hidden h-px w-6 -translate-y-1/2 translate-x-full bg-[#3A2119]/10 lg:block" />
                )}
              </div>
            );
          })}
